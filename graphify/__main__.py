...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --backend B             claude|claude-cli|bedrock (default: whichever API key is set)")
    ...
    if cmd == "install":
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: graphify extract <path> [--backend claude|claude-cli|bedrock] "
        ...
        if backend is None:
            ...
            if backend is None:
                print(
                    "error: no LLM API key found. Set ANTHROPIC_API_KEY (claude), "
                    "or use --backend claude-cli (requires `claude` CLI) or --backend bedrock.",
                    file=sys.stderr,
                )
                sys.exit(1)
